...
function AICore:RandomEnemyHeroInRange( entity, range )
	local enemies = FindUnitsInRadius( entity:GetTeamNumber(), entity:GetOrigin(), nil, range, DOTA_UNIT_TARGET_TEAM_ENEMY, 19, 128, 0, false )
	if #enemies > 0 then
...
function AICore:WeakestEnemyUnitInRange( entity, range )
	local enemies = FindUnitsInRadius( entity:GetTeamNumber(), entity:GetOrigin(), nil, range, DOTA_UNIT_TARGET_TEAM_ENEMY, 19, 128, 0, false )
...
function AICore:CreateBehaviorSystem( behaviors )
	...
	-- print("Possible behaviors", #BehaviorSystem.possibleBehaviors)
	...
	function BehaviorSystem:Think()
		if GameRules:GetGameTime() >= self.currentBehavior.endTime then
			...
			-- print("new behavior", dump(newBehavior.name))
		...
		-- print("current behavior", dump(self.currentBehavior.name), self.currentBehavior.order.OrderType)
		...
		if self.currentBehavior.order and self.currentBehavior.order.OrderType ~= DOTA_UNIT_ORDER_NONE then
			if self.repeatedlyIssueOrders or
				...
				-- print("Bing: new Order?", self.currentBehavior.name)
	...
	function BehaviorSystem:ChooseNextBehavior()
		...
		-- print("Bing: behavior result?", result.name, bestDesire)
		return result
	end
